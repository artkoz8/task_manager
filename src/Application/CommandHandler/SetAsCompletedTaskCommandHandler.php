...
final class SetAsCompletedTaskCommandHandler
{
    ...
    public function __invoke(SetAsCompleteTaskCommand $command): ?TaskReadModel
    {
        $task = $this->taskRepository->getByTaskId($command->taskId);

        $task->changeStatus(TaskStatus::COMPLETED, $this->taskStatusStrategyResolver);
